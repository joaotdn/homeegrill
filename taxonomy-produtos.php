<?php
  //Header
  include_once "header.php";

  //breadcrumb
  include_once "content-breadcrumb.php";
  ?>
  
  <div class="row">
    
    <header class="small-12 columns post-header">
      <h2 class="no-margin lh-normal text-low">Churrasqueiras a gás linha Smart</h2>
      <div class="divide-20"></div>

      <nav class="share-post small-12 left show-for-medium-up">
        <img src="media/share.png" alt="" class="left">
      </nav><!-- // share buttons -->

    </header><!-- //post-header -->

    <div id="showroom-carousel" class="small-12 left products-archive-list">
                  
      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>

      <!-- item -->
      <div class="item rel tiny-12 small-6 medium-4 large-4 columns">
        <figure class="small-12 left rel showroom-thumb">
          <a href="#" class="d-block small-12 left text-center">
            <img src="media/img1.png" alt="">
          </a>
        </figure>

        <article class="infocap inner abs small-12">
          <div class="small-12 left fix-bg">
            <div class="container ghost small-12 left">
                    
              <header class="small-12 left d-table">
                <hgroup class="small-12 d-table-cell">
                  <h3 class="font-lite no-margin lh-normal">Churrasqueira a gás</h3>
                  <h3 class="font-bold no-margin lh-normal">home $ grll PREMIUM COMPACT GRILL HG-2B</h3>
                </hgroup>
              </header>

              <p class="d-table font-small small-12 left no-margin">
                <span class="d-table-cell small-12">
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                  <span class="small-12 left">• Fabricado em ABS</span>
                </span>
              </p>
                    
              <a href="#" title="" class="button small-12 left font-xlarge font-bold no-margin">ver detalhes <span class="icon-chevron-circle-right font-large"></span></a>
            </div>
          </div>
        </article>
      </div>
            
    </div> <!-- fim da listagem -->

    <!-- paginação -->
    <div class="small-12 columns showroom-controls">
      <ul class="inline-list no-margin">
        <li>
          <a href="#" class="btn d-table prev-churrasqueira">
            <span class="icon-arrow-left d-table-cell"></span>
          </a>
        </li>

        <li>
          <a href="#" class="btn d-table next-churrasqueira">
            <span class="icon-uniE603 d-table-cell"></span>
          </a>
        </li>

        <li>
          <h5 class="text-low" style="margin-top:20px;"><span>1 de 8 páginas</span></h5>
        </li>
      </ul>

      <div class="divide-30"></div>
    </div>

  </div><!-- // row -->

<?php
  //footer
  include_once "footer.php";
?>
